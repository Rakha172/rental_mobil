                ...
                <div class="card-body">
                    <form action="" method="GET">
                        <div class="col-sm-10 d-flex">
                            <a href="{{ route('vehicle.create') }}" class="btn btn-secondary"
                                style="margin-right: 8px; color:black;">Add</a>
                                ...
                                @if (isset($vehicle))
                                    @foreach ($vehicle as $index => $vhcle)
                                        <tr>
                                            ...
                                            <td class="d-flex">
                                                <form action="{{ route('vehicle.destroy', $vhcle->id) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                                    @csrf
                                                    @method('delete')
                                                    <a href="{{ route('vehicle.edit', $vhcle->id) }}"
                                                        class="btn btn-secondary"
                                                        style="height: 40px; color: black;">Edit</a>
                                                    <input name="_method" type="hidden" value="DELETE">
                                                    <button class="btn ml-2 btn-secondary"
                                                        style="height: 40px; color: black;">Delete</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
