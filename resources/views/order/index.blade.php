                ...
                <form action="" method="GET">
                    <div class="row mb-2">
                        <div class="col-sm-10 d-flex">
                            <input type="text" placeholder="Please input key for search data" name="search" autofocus
                                style="border-radius:5px; width:700px; margin-bottom:1rem;" value="{{ $search }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('order.destroy', $ordr->id) }}"method="POST">
                                            ...
                                            @method('delete')
                                            @if ($payment->count() > 0)
                                                <a href="{{ route('payment.edit', $ordr->id) }}"
                                                    class="btn ml-2 btn-secondary"
                                                    style="height:40px; color:black;">Payment</a>
                                            @endif
                                            <button class="btn ml-2 btn-secondary"
                                                style="height:40px; color:black;">Delate</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
